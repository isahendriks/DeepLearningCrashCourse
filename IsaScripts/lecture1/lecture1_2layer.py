...
def dnn3_class(wa, wb, wc, x):
    return(sigmoid(ReLU(ReLU(x@wa)@wb)@wc))
...
filename = "data_class_2d_convex_clean.csv"
...
#%% Define starting parameters for 3 layers

num_neurons_1 = 7 # number of neurons in the hidden layer
num_neurons_2 = 5 # number of neurons in second 
rng  = default_rng()
...
plot_pred_2d(x, y_gt, y_p=dnn3_class(wa, wb, wc, x))
...
for i in range(num_train_iterations):
    ...
    delta_c = d_error * d_sigmoid(p_c)
    wc = wc - eta * delta_c * np.transpose(x_selected_c)

    delta_b = np.sum(wc * delta_c, axis=1)*d_ReLU(p_b)
    wb = wb - eta * delta_b  * np.transpose(x_selected_b)
